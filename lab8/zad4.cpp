...
Да се напише функциjа pecatiInstrumenti(ZicanInstrument &zi, ZicanInstrument **i, int n)
 ...
 Функциjата jа печати цената на сите жичани инструменти од низата кои имаат ист броj на жици како и инструментот проследен како прв аргументна функциjата.
*/
#include "iostream"
#include "cstring"
using namespace std;
...
protected:
    ...
    float osvnovnaCena;
public:
    ZicanInstrument(char *name="", int brojZhici=0, float osvnovnaCena=0){
        strcpy(this->ime,ime);
        this->brojZhici=brojZhici;
        this->osvnovnaCena=osvnovnaCena;
    }
    ZicanInstrument(const ZicanInstrument &z){
        strcpy(this->ime,z.ime);
        this->brojZhici=z.brojZhici;
        this->osvnovnaCena=z.osvnovnaCena;
    }
    bool operator==(ZicanInstrument &z) const{
        return brojZhici == z.brojZhici;
    }

    friend ostream &operator<<(ostream &out, ZicanInstrument &z){
        out << z.ime << " " << z.brojZhici << " " << z.osvnovnaCena << endl;
...
private:
    char forma[21];
public:
    Mandolina(char *name="", int brojZhici=0, float osvnovnaCena=0, char *forma=""): ZicanInstrument(name,brojZhici,osvnovnaCena){
        strcpy(this->forma,forma);
    }
    double cena()const override{
        if (!strcmp("Neapolitan", forma)){
            return 1.15 * osvnovnaCena;
        }else{
            return osvnovnaCena;
        }
    }
...
private:
    double size;
public:
    Violina(char *name="", int brojZhici=0, float osvnovnaCena=0,double size =0):ZicanInstrument(name,brojZhici,osvnovnaCena){
        this->size=size;
    }
    double cena()const override{
        if (size == 0.25){
            return 1.1 * osvnovnaCena;
        }else if (size == 1 ){
            return  1.2 * osvnovnaCena;
        }else{
            return osvnovnaCena;
        }
    }
