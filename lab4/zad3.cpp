...
//
//
//Надвор од класата да се дефинира метод void findBestAgency(Agency *agencies, int numAgencies) кој прима низа
// од агении и ја печати онаа агенција која остварила најмногу профит од сите тури. Овој метод мора да пристапува до
// самите приватни полиња од класите.
#include "iostream"
#include "cstring"
using namespace std;
class Tour{
...
void findBestAgency(Agency *agencies, int numAgencies){
    ...
    for (int i = 0; i < numAgencies; ++i) {
        ...
        for (int j = 0; j < numAgencies; ++j) {
            totalProfit+=agencies[i].tours[j].profitByTour();
        }
    ...
    cout<<"Best agency: "<<agencies[index].name<<" with profit "<<maxProfit<<endl;
}
int main() {
...
//    return 0;
//}
//Ednata bese ista kako drugite termini drugata so countries so sortanje po gdp no pred toa da gi delish vo Republicans i Democrats
